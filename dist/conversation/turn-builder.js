 ...
 */
export function buildTurnsFromSegments(segments, steps, turnBoundaries = []) {
...
function buildBoundaryTurns(segments, steps, turnBoundaries) {
    const edges = [...new Set(turnBoundaries.filter((ms) => Number.isFinite(ms)))].sort((a, b) => a - b);
    const sorted = resolveLiveSpeakers(segments, steps)
        .map((segment) => ({ ...segment }))
        .sort((a, b) => a.start_ms - b.start_ms || a.end_ms - b.end_ms);
    const spanIndex = (ms) => {
    ...
    for (const seg of sorted) {
        ...
        if (index >= edges.length)
            span.end_ms = Math.max(span.end_ms, seg.end_ms);
    }
    return [...spans.entries()]
        ...
        void speakerWeights;
        const covering = steps.find((step) => step.timestamp_ms < turn.end_ms && step.timestamp_ms + 1000 > turn.start_ms);
        return {
            ...turn,
